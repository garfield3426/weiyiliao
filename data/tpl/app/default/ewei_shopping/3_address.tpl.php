...
<div class="head">
	...
	<a href="javascript:;" class="bn pull-right"><i class="fa fa-shopping-cart"></i><span class="buy-num img-circle"> <?php  echo $carttotal;?> </span></a>
</div>
...
<form class="form-horizontal" method="post" role="form">
	...
	<div class="order-main">
		<div id="myaddress">
		<?php  if(is_array($addresses)) { foreach($addresses as $row) { ?>
			<div id='address_<?php  echo $row['id'];?>' class="<?php  if($row['isdefault']==1) { ?>shopcart-main1<?php  } else { ?>shopcart-main<?php  } ?> img-rounded address_item" style='margin:0;padding:10px;margin-bottom:10px;cursor:pointer' onclick='editAddress(<?php  echo $row['id'];?>,<?php echo $row['isdefault']==1?'1':'0'?>)'>
				<span><?php  echo $row['province'];?> <?php  echo $row['city'];?> <?php  echo $row['district'];?> <?php  echo $row['address'];?> <br/> <?php  echo $row['username'];?> - <?php  echo $row['mobile'];?></span><br/>
				<span>
					<?php  if(!$row['isdefault']) { ?><a href="javascript:;" onclick="saveDefaultAddress(event,<?php  echo $row['id'];?>)">设为默认</a><?php  } ?>
					<a href="javascript:;" onclick="removeAddress(event,<?php  echo $row['id'];?>)"><i class="fa fa-remove"></i> 删除</a>
				</span>
			</div>
		<?php  } } ?>
		</div>
		<div><button type="button" class="btn btn-danger" onclick="addAddress()"><i class="fa fa-plus"></i> 添加修改地址</button></div>
		<div class="add-address img-rounded" id="addAddressPanel" <?php  if(!empty($addresses)) { ?> style="display:none;"<?php  } ?>>
...
<script type="text/javascript">
	<?php  if(empty($addresses) && $_W['account']['level'] == ACCOUNT_SERVICE_VERIFY) { ?>
		wx.ready(function () {
			wx.openAddress({
		...
		<?php  } ?>
	var from='<?php  echo $_GPC['from'];?>';
	var returnurl = '<?php  echo $_GPC['returnurl'];?>';
	cascdeInit('','','');
