...
<div class="shopping-main">
	...
	<div class="list" id="list">
		...
		<div <?php  if($sort==2) { ?>class='sel'<?php  } else { ?>class="nosel"<?php  } ?> style='border:1px solid #e9342a;margin-left:-1px;float:left;width:25%;text-align: center;' onclick="location.href='<?php  echo $sorturl;?>&sort=2&sortb2=<?php  echo $sortb22;?>'">
			按人气 <?php  if($sort==2) { ?><?php  if($sortb2=="desc") { ?><i class="fa fa-arrow-down"></i><?php  } else { ?><i class="fa fa-arrow-up"></i><?php  } ?><?php  } ?>
		</div>
...
<div class="list-item img-rounded">
	<div>
		...
		<span class="title">
			...
			<strong class="text-danger" style="font-size: 14px;">￥<?php  echo $item['marketprice'];?></strong>
			<?php  if($item['originalprice'] != '0.00') { ?><strong style="color: #b0b8bd; text-decoration: line-through;">￥<?php  echo $item['originalprice'];?></strong><?php  } ?>
		</span>
...
<?php  if(empty($_W['isajax'])) { ?>
	...
	<div class="show-more"><a href="javascript:;" data-pindex="2" onclick="loadPage('<?php  echo $pindex;?>', 'list')" class="img-rounded" id="pager">浏览更多商品</a></div>
</div>
